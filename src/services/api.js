...
export const client = async (config) => {
  let getAuthData = await AsyncStorage.getItem('auth');
  ...
  const configClient =
    config.type === 'm' ? {mutation: config.schema} : {query: config.schema};
  if (config.data !== null) {
    configClient.variables = config.data;
  }

  if (config.type === 'm') {
    return apolloClient.mutate(configClient);
  }
